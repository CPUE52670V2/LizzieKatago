...
public class BoardData {
    ...
    public BoardData(Stone[] stones, Optional<int[]> lastMove, Stone lastMoveColor, boolean blackToPlay, Zobrist zobrist, int moveNumber, int[] moveNumberList, int blackCaptures, int whiteCaptures, double winrate, int playouts) {
        this.moveMNNumber = -1;
    ...
    public Double preLeadBorderKomi=null;
    ...
    public void tryToSetBestMoves(List<MoveData> moves, String engName, boolean isFromLeelaz, int totalplayouts, ArrayList<Double> estimateArray) {
        ...
        if (!(EngineManager.isEngineGame && EngineManager.engineGameInfo.isGenmove)) wrn = Lizzie.leelaz.wrn;
        // 排序
        Collections.sort(moves, new Comparator<MoveData>() {
    ...
    private void tryToLimitMoves(List<MoveData> moves, List<MoveData> lastMoves, boolean isMain) {
        ...
        if (Lizzie.config.limitMaxSuggestion > 0 && !Lizzie.config.showNoSuggCircle && (moves.size() > Lizzie.config.limitMaxSuggestion)) {
    ...
    public void tryToSetBestMoves2(List<MoveData> moves, String engName, boolean isFromLeelaz, int totalplayouts, ArrayList<Double> estimateArray) {
    ...
    public String bestMovesToString() {
        ...
        for (MoveData move : bestMoves) {
            ...
            if (isKataData) sb.append(" scoreMean ").append(String.format(Locale.ENGLISH, "%.2f", move.scoreMean));
