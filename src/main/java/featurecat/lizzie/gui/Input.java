...
public class Input implements MouseListener, KeyListener, MouseWheelListener, MouseMotionListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.isAltDown() && !SwingUtilities.isMiddleMouseButton(e) && (LizzieFrame.allowcoords != "" || LizzieFrame.avoidcoords != ""))
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (tempDrag) {
            ...
            // 大棋盘手工落子@dongxiaoming
            if (e.getButton() == MouseEvent.BUTTON1) {// left click
                if (e.getClickCount() == 2 && !Lizzie.frame.isTrying && !Lizzie.frame.isPlayingAgainstLeelaz && !Lizzie.frame.isAnaPlayingAgainstLeelaz && Lizzie.config.allowDoubleClick) { // TODO: Maybe need to delay check
                    Lizzie.frame.onDoubleClicked(Utils.zoomOut(e.getX()), Utils.zoomOut(e.getY()));
                } else {
                    if (Lizzie.frame.tryToMarkup(Utils.zoomOut(e.getX()), Utils.zoomOut(e.getY()))) {
                        return;
                    }
                    if (insert == 0) {
                        ...
                        if (Lizzie.frame.iscoordsempty(Utils.zoomOut(e.getX()), Utils.zoomOut(e.getY()))) {
                            int[] coords = Lizzie.frame.convertmousexytocoords(Utils.zoomOut(e.getX()), Utils.zoomOut(e.getY()));
                ...
                if (Lizzie.frame.onClickedRight(Utils.zoomOut(e.getX()), Utils.zoomOut(e.getY()))) {
                    return;
                }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (Draggedmode && !Lizzie.frame.isTrying && !LizzieFrame.urlSgf && !Lizzie.frame.isPlayingAgainstLeelaz && !Lizzie.frame.isAnaPlayingAgainstLeelaz && Lizzie.config.allowDrag) {
            Lizzie.frame.DraggedReleased(Utils.zoomOut(e.getX()), Utils.zoomOut(e.getY()));
            return;
        }
    ...
    private void goCommentNode(boolean moveForward) {
        if (moveForward) {
            ...
            LizzieFrame.undo(Lizzie.board.getHistory().getCurrentHistoryNode().goToPreviousNodeWithComment());
        }
    }
        ...
        switch (e.getKeyCode()) {
            ...
            case VK_S:
                if (e.isControlDown() && e.isShiftDown()) {
                    ...
                    Lizzie.frame.saveImage(Lizzie.frame.statx, Lizzie.frame.staty, (int) (Lizzie.frame.grw * 1.03), Lizzie.frame.grh + Lizzie.frame.stath);
                } else {
                    if (e.isAltDown()) {
            ...
            case VK_V:
                ...
                if (controlIsPressed(e)) {
                    ...
                    Lizzie.config.uiConfig.put("show-suggestion-variations", Lizzie.config.showSuggestionVariations);
                } else {
            ...
            case VK_X:
                ...
                //        }
                    ...
                    if (!Lizzie.frame.showControls) {
                        ...
                        if (Lizzie.frame.listScrollpane.isVisible()) Lizzie.frame.listScrollpane.setVisible(false);
...
//        鼠标滚轮回退事件
        ...
        if (e.getWhen() - wheelWhen > 0) {
            ...
            if (e.getWheelRotation() > 0) {
                if (LizzieFrame.boardRenderer.isShowingBranch() || (Lizzie.config.isDoubleEngineMode() && LizzieFrame.boardRenderer2.isShowingBranch())) {
                    Lizzie.frame.doBranch(1);
                    Lizzie.frame.refresh();
                } else {
                    redo();
